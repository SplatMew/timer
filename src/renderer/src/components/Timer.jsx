  ...
  return (
    <div>
      {isEditing ? (
        <div className="flex justify-center">
          <div>
            <InputField
              label={'Hours'}
              value={hours}
              onChange={(e) => {setHours(parseInt(e.target.value)); setsavedHours(e.target.value)}}
            ></InputField>
            <InputField
              label={'Minutes'}
              value={minutes}
              onChange={(e) => {setMinutes(parseInt(e.target.value)); setSavedMinutes(e.target.value)}}
            ></InputField>
            <InputField
              label={'Seconds'}
              value={seconds}
              onChange={(e) => {setSeconds(parseInt(e.target.value)); setSavedSeconds(e.target.value)}}
            ></InputField>
            <button 
        ...
        <div>
          <div className='flex justify-center'>
            <h1 className=' text-6xl text-slate-200'> {`${hours.toString().padStart(2,"0")}:${minutes.toString().padStart(2,"0")}:${seconds.toString().padStart(2,"0")}`}</h1>
          </div>
          <div id = "timer-button" className='flex justify-center text-slate-300 text-4xl '>
            {isCountingDown ? (
              <>
              <button className={!isOverlay ? 'm-2 hover:text-slate-50' : 'hidden'}
